...
![预测下一个 token](../images/00Summary01ScalingLaw/01ScalingLaw04.png)


## 核心发现：模型参数、数据集和计算量对模型性能的影响

Kaplan et al.于 2020 年发表的论文[Scaling Laws for Neural Language Models](https://arxiv.org/pdf/2001.08361)是 LLM 缩放定律领域的奠基性工作。该研究通过在 WebText2 数据集上训练 Transformer 模型，首次系统性地揭示了语言模型性能与模型规模、数据集大小和计算量之间的精确幂律关系。
...
计算量 $C \approx 6NBS$ 。其中 $B$ 表示 batch size， $S$ 表示训练步长。 $L(C_{\text{min}})$ 的图与其他两个图的不同在于，其是由多个曲线的最低点相连得出的。这多个曲线就是不同 batch size 下，模型随训练步长的变化。这里在展示的是要让模型到达指定性能需要最少投入的计算资源。从另一面看， $L(C_{\text{min}})$ 展示了在计算资源固定的前提下，模型性能能收敛到哪里。

![模型性能与计算量、数据集大小、模型规模之间的关系](../images/00Summary01ScalingLaw/01ScalingLaw01.png)
...
![batch size](../images/00Summary01ScalingLaw/01ScalingLaw07.png)

在充分大 batch size 下，训练步长与模型性能间存在幂律关系，且此规律在不同参数规模的模型架构中均呈现一致性。

![训练步长](../images/00Summary01ScalingLaw/01ScalingLaw08.png)
...
![模型参数与数据](../images/00Summary01ScalingLaw/01ScalingLaw03.png)
![模型参数与数据](../images/00Summary01ScalingLaw/01ScalingLaw05.png)
...
![样本效率](../images/00Summary01ScalingLaw/01ScalingLaw02.png)
...
![作者使用三种不同方法的预测最优模型大小，发现当时的大模型训练严重不足。这也导致 2021 年大家复现 GPT3 都没有做出特别好的效果。](../images/00Summary01ScalingLaw/01ScalingLaw06.png)
...
![最优学习率和 batch size](../images/00Summary01ScalingLaw/01ScalingLaw09.png)
...
![最优学习率和 batch size](../images/00Summary01ScalingLaw/01ScalingLaw10.png)
...
![上下文长度](../images/00Summary01ScalingLaw/01ScalingLaw11.png)
...
![数据分布偏移](../images/00Summary01ScalingLaw/01ScalingLaw12.png)
