...
这个模型非常简单，只包含一个线性层，对应我们要学习的 y = wx + b 关系。
...
单卡训练函数首先设置了核心超参数：batch size 为 32（这是深度学习中常用的批量大小，兼顾计算效率与梯度稳定性），学习率为 0.01（适合 SGD 优化器在小数据集上的收敛），训练轮数为 10（足够让线性模型收敛到接近真实值的参数）。
...
1. 模型必须能够放入单个 GPU 内存中，因为每个 GPU 都需要保存完整模型
