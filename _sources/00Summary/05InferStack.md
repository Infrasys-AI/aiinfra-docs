...
当大模型从实验室走向产业应用，能跑通已不再是目标，**如何让模型在成本可控的前提下，以低延迟、高吞吐的姿态稳定服务**，成为决定落地成败的关键。这一转变背后，大模型推理的技术正经历深刻重构：传统推理引擎被新一代大模型推理引擎取代，大模型推理算力向云端大算力+端侧轻算力异构延伸。

更重要的是，大模型推理早已不是单点优化，从模型压缩到输出采样的全流程，必须与 AI Infra 的算力-软件-存储分层架构深度绑定，才能构建起大模型推理效率的护城河。

## 1. 推理与 AI Infra 关系

![AIInfra](../images/00Summary/05InferStack01.png)
...
![AIInfra](../images/00Summary/05InferStack02.png)
...
![](../images/00Summary/05InferStack03.png)
...
![](../images/00Summary/05InferStack04.png)
...
![](../images/00Summary/05InferStack05.png)
...
![](../images/00Summary/05InferStack06.png)
...
ONNX Runtime（ORT）的核心优势是跨框架与跨硬件兼容性，支持将 PyTorch、TensorFlow 等框架训练的模型转换为 ONNX 格式后直接部署。通过插件机制适配多种硬件，但是随着 TensorFLow 等越来越多的训练框架开始消失，ONNX 越来越少任用。
...
MoE 通过稀疏激活专家层降低计算量，但推理时面临两大挑战：一是**路由策略**需精准选择适配请求的专家，二是**专家均衡**需避免部分专家过载。这两点直接决定 MoE 模型的推理性能与服务稳定性。
...
专家均衡的关键是避免部分专家过载。由于请求特征分布不均，部分专家可能被频繁选中，导致其处理延迟飙升至其他专家的 3 倍以上。
...
![](../images/00Summary/05InferStack10.png)
...
![](../images/00Summary/05InferStack08.png)
...
![](../images/00Summary/05InferStack09.png)
...
观测是发现问题，测试是预防风险，治理是长期可控。避免因大模型推理过程中的指标盲区、未测故障或合规漏洞，导致服务中断或业务风险。

### 7.1. 观测指标

大模型推理服务核心指标的选择与解读，直接决定能否精准定位瓶颈、优化资源、控制成本。

**latency（延迟）** 需关注分位值而非平均值：平均延迟易掩盖极端情况，重点监控 P95、P99、P999 延迟，同时拆分预处理延迟-计算延迟-输出延迟定位瓶颈。此外，端侧场景需额外监控首 Token 延迟。

**throughput（吞吐量）** 需按 token 量而非请求数统计：大模型推理的核心产出是 token 生成，相同请求数下，8K 长序列与 1K 短序列的算力消耗差异达 64 倍。可以用每秒 token 数（token/s）衡量吞吐量。同时监控实际 batch size/最大可支持 batch size，调整动态 Batch 策略。

**GPU util（GPU 利用率）** 需区分计算利用率与显存利用率：计算利用率低可能是 Batch 过小或任务调度不及时；显存利用率低可能是 KV Cache 管理不当，例如未启用 PagedAttention 导致内存碎片。端侧 NPU 需额外监控功耗利用率，避免推理时功耗过高导致设备发热。

**cost per token（单 token 成本）** ，需拆解硬件成本+软件优化增益，硬件成本除以单日 token 产出，得出基础成本；Infra 层优化可降低成本。工程中需按业务线统计成本，避免高成本场景占用过多资源。

### 7.2. 模型治理

大模型推理的治理在版本管理避免迭代风险，审计确保可溯源，合规符合法律法规，访问控制防止滥用。

**版本管理** 需实现灰度发布与快速回滚：工程中通过模型版本标识+流量切分管理迭代，如新推理大模型版本上线时先切 10%流量，监控 latency、精度无异常后再逐步扩量；若发现新版本精度下降，可在分钟内回滚至旧版本。同时需管理推理引擎、核心库等依赖版本，避免因依赖升级导致兼容性问题。

**审计** 需记录全链路操作日志：包括模型调用日志，谁调用、调用时间、输入输出数据、token 量；运维操作日志，谁更新模型、谁调整调度策略。
