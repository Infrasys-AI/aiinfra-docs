...
![训练曲线包络](../images/00Summary/01ScalingLaw14.png)
...
![IsoFLOP](../images/00Summary/01ScalingLaw15.png)

### 3.3 拟合参数化损失函数

最后一种方法则试图直接拟合一个 loss 与数据量和模型规模的函数（见左下图）。根据图中的虚线可以绘制出上一个方法的 IsoFLOP 曲线。但可以看到拟合的结果不如上一个方法的好。

![拟合参数化损失函数](../images/00Summary/01ScalingLaw16.png)

下表展示了不同方法的拟合结果，可以看出第三个方法的结果和前两个的不太一致。

![拟合结果](../images/00Summary/01ScalingLaw17.png)
...
![提示任务](../images/00Summary/01ScalingLaw18.png)

下图展示了模型在不同任务上随模型规模的性能变化，可以看到在突破规模的临界点后，表现大幅度提升。

![提示任务性能](../images/00Summary/01ScalingLaw19.png)
...
![增强提示](../images/00Summary/01ScalingLaw20.png)
